...
public class AttendanceReportTests
{
    ...
    public void ExportCsv_Returns_NonEmpty_Bytes_With_Headers()
    {
        ...
        Assert.Contains("Date,EmployeeCode,EmployeeName,Department,Branch,CheckIn,CheckOut,WorkedHours,OvertimeHours,OvertimePay,EarlyDepartureMinutes,Status,Notes", text);
    ...
    public void ExportCsv_Handles_Null_CheckIn_CheckOut()
    {
        ...
        Assert.Equal("Absent", row[11]); // Status (col 11 — after OvertimeHours + OvertimePay + EarlyDepartureMinutes)
    }
    ...
    public void ExportXlsx_Can_Be_Opened_With_ClosedXML()
    {
        ...
        Assert.Equal("EarlyDepartureMinutes", detail.Cell(1, 11).Value.ToString()); // col 11 — after OvertimeHours + OvertimePay
        Assert.Equal("Status", detail.Cell(1, 12).Value.ToString()); // col 12 — after EarlyDepartureMinutes
